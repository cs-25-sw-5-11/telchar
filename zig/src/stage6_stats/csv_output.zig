    ...
    for (matched_trips) |matched_trip| {
        ...
        while (i < projections.len) {
            ...
            if (end_idx > start_idx) {
                ...
                // Only record if we have positive time and moved along the edge
                if (time_diff > 0) {
...
pub fn writeAllCsvFiles(
    ...
    try writeEdgeDataCsv(allocator, matched_trips, @intCast(network.edges.len), time_interval_sec, edge_data_path);
