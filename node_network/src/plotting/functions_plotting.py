...
def plot_networks(
    ...
    for edge in tqdm(Edge.get_all_edges(), desc="Plotting edges"):
        lats = (
            [edge.start.lat]
            + [lat for lat, lon, id in edge.non_vertex_nodes]
            + [edge.end.lat]
        )
        lons = (
            [edge.start.lon]
            + [lon for lat, lon, id in edge.non_vertex_nodes]
            + [edge.end.lon]
        )
    ...
    for vertex in tqdm(
        Vertex.get_all_vertices(), desc="Collecting vertices for scatter plot"
    ):
    ...
    if show_densest_bin:
        ...
        if max_count > 0:
            ...
            print(
                f"Densest bin center: lat={center_lat}, lon={center_lon}, count={max_count}"
            )
            print(
                f"Edges in densest bin: {[e.id for e in Edge._bin_lookup[(max_i, max_j)]]}"
            )
            plt.scatter(
                [center_lon],
                [center_lat],
    ...
    if highlight_lat is not None and highlight_lon is not None:
        highlight_edges = get_edges_near_coordinates(
            highlight_lat, highlight_lon, cell_range
        )
        plt.scatter(
            [highlight_lon],
            [highlight_lat],
...
def plot_directed_graph(
    ...
    lon_min=None,
    lon_max=None,
    trip_point_lats=None,
    trip_point_lons=None,
    max_dist=None,
    ...
    def add_direction_arrows(edge, lats, lons, meters_per_arrow=100, max_arrows=10):
        ...
        num_arrows = max(
            1, int(edge_length_m / meters_per_arrow)
        )  # At least 1 arrow, 1 per 100m
    ...
    def get_relevant_vertex_bins():
        ...
        if lat_max is not None:
            max_lat_bin = min(
                int((LAT_MAX - LAT_MIN) / LAT_BIN_SIZE),
                int((lat_max - LAT_MIN) / LAT_BIN_SIZE) + 1,
            )
        else:
        ...
        if lon_max is not None:
            max_lon_bin = min(
                int((LON_MAX - LON_MIN) / LON_BIN_SIZE),
                int((lon_max - LON_MIN) / LON_BIN_SIZE) + 1,
            )
        else:
    ...
    print(
        f"Found {len(vertices_to_plot)} vertices in {len(relevant_bins)} relevant bins"
    )
    print(
        f"Plotting {len(edges_to_plot)} edges connected to those vertices (instead of {Edge.get_num_of_edges()} total edges)"
    )
