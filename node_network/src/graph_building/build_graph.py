...
def create_edge(road: dict[str, any], nodes_dict: dict[str, dict[str, float]]) -> None:
    ...
    start_vertex = Vertex(
        start_node_data["lat"], start_node_data["lon"], int(start_node_id)
    )
...
def convert_point_to_vertex(
    node_id: str, nodes_dict: dict[str, dict[str, float]]
) -> None:
...
def print_stats():
    ...
    print(
        f"Created {Vertex.get_num_of_vertices()} vertices and {Edge.get_num_of_edges()} edges"
    )
    return
