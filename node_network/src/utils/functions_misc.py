...
def restore_network_to_original_state(debug: bool = False):
    ...
    if (
        remaining_temp_vertices == 0
        and remaining_temp_edges == 0
        and remaining_detached_edges == 0
    ):
...
def validate_network_integrity():
    ...
    for edge in Edge.get_all_edges():
        if Vertex.check_vertex_exists(edge.start.id) is False:
            issues.append(
                f"Edge {edge.id} start vertex {edge.start.id} not in vertex_dict"
            )

        if Vertex.check_vertex_exists(edge.end.id) is False:
            issues.append(
                f"Edge {edge.id} end   vertex {edge.end.id} not in vertex_dict"
            )

        # Check vertex-edge connections are consistent
        if edge not in edge.start.onward_edges:
            issues.append(
                f"Edge {edge.id} missing from start vertex {edge.start.id} onward_edges"
            )

        if edge not in edge.end.backward_edges:
            issues.append(
                f"Edge {edge.id} missing from end vertex {edge.end.id} backward_edges"
            )
        ...
        if not edge.oneway:
            if edge not in edge.end.onward_edges:
                issues.append(
                    f"Bidirectional edge {edge.id} missing from end vertex {edge.end.id} onward_edges"
                )

            if edge not in edge.start.backward_edges:
                issues.append(
                    f"Bidirectional edge {edge.id} missing from start vertex {edge.start.id} backward_edges"
                )
    ...
    for vertex in Vertex.get_all_vertices():
        for edge in vertex.onward_edges.keys():
            if Edge.check_edge_exists(edge.id) is False:
                issues.append(
                    f"Vertex {vertex.id} references non-existent edge {edge.id} in onward_edges"
                )

        for edge in vertex.backward_edges.keys():
            if Edge.check_edge_exists(edge.id) is False:
                issues.append(
                    f"Vertex {vertex.id} references non-existent edge {edge.id} in backward_edges"
                )
...
def compare_network_states(state1, state2):
    ...
    if state1["vertex_count"] != state2["vertex_count"]:
        differences.append(
            f"Vertex count: {state1['vertex_count']} -> {state2['vertex_count']}"
        )

    if state1["edge_count"] != state2["edge_count"]:
        differences.append(
            f"Edge count: {state1['edge_count']} -> {state2['edge_count']}"
        )
    ...
    if state1["temporary_vertices"] != state2["temporary_vertices"]:
        differences.append(
            f"Temporary vertices: {state1['temporary_vertices']} -> {state2['temporary_vertices']}"
        )

    if state1["temporary_edges"] != state2["temporary_edges"]:
        differences.append(
            f"Temporary edges: {state1['temporary_edges']} -> {state2['temporary_edges']}"
        )

    if state1["detached_edges"] != state2["detached_edges"]:
        differences.append(
            f"Detached edges: {state1['detached_edges']} -> {state2['detached_edges']}"
        )
    ...
    for vertex in common_vertices:
        if (
            state1["vertex_connections"][vertex.id]
            != state2["vertex_connections"][vertex.id]
        ):
