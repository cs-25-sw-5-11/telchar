...
class PointProjection:
    ...
    def __init__(
        ...
        self.backward_vertex_dist: float = (
            self.parent_edge.get_projection_distance_from_start(
                self.seg_idx, self.seg_t
            )
        )
        self.onward_vertex_dist: float = (
            self.parent_edge.length - self.backward_vertex_dist
        )

    def get_distance_between_projections(
        self, network: Network, second_projection: "PointProjection"
    ) -> float:
        ...
        for first_vertex in first_vertices:
            ...
            for second_vertex in second_vertices:
                ...
                min_dist = min(
                    min_dist,
                    dist + distance_to_first_vertex + distance_to_second_vertex,
                )
    ...
    def get_distance_between_projections_sharing_edge(
        ...
        if self.parent_edge.oneway:
            if self.backward_vertex_dist < second_projection.backward_vertex_dist:
                return (
                    second_projection.backward_vertex_dist - self.backward_vertex_dist
                )
            else:
                return None
        else:
            return abs(
                self.backward_vertex_dist - second_projection.backward_vertex_dist
            )
    ...
    def get_distance_to_vertex(self, network: Network, target_vertex: Vertex) -> float:
        ...
        if self.parent_edge.oneway:
            return (
                network.get_distance(self.onward_vertex, target_vertex)
                + self.onward_vertex_dist
            )
            ...
            return min(
                network.get_distance(self.onward_vertex, target_vertex)
                + self.onward_vertex_dist,
                network.get_distance(self.backward_vertex, target_vertex)
                + self.backward_vertex_dist,
            )

    def get_distance_from_vertex(
        self, network: Network, source_vertex: Vertex
    ) -> float:
        """Get the distance in meters from the given vertex to this projection."""
        if self.parent_edge.oneway:
            return (
                network.get_distance(source_vertex, self.backward_vertex)
                + self.backward_vertex_dist
            )
            ...
            return min(
                network.get_distance(source_vertex, self.backward_vertex)
                + self.backward_vertex_dist,
                network.get_distance(source_vertex, self.onward_vertex)
                + self.onward_vertex_dist,
            )
