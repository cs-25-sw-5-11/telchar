...
from config import LAT_COLUMN, LON_COLUMN, TIMESTAMP_COLUMN, TRIP_ID_COLUMN
...
def main():
    ...
    all_speeds = []
    target_trip_id = (
        None  # Set to a string trip_id to filter, or None to disable filtering
    )
...
def process_file(file, target_trip_id=None):
    ...
    with open(file, "r", encoding="utf-8") as f:
        header = f.readline()
        _columns = [col.strip() for col in header.strip().split(",")]
        ...
        for line in f:
            ...
            if (
                ...
                try:
                    ...
                    if diff == 0:
                        print(
                            f"Zero time difference detected in file {file}, trip_id {trip_id}, at line {line_num} (timestamp={timestamp})"
                        )
                    lon1 = float(lon)
                    lat1 = float(lat)
                    lon0 = float(prev_lon)
                    ...
                    if diff > 0:
                        ...
                        if speed > 100:
                            print(
                                f"High speed detected: {speed:.2f} km/h in file {file}, trip_id {trip_id}, at line {line_num}"
                            )
                except Exception:
                    pass
            prev_trip_id = trip_id
