...
for csv_file in csv_files:
    try:
        ...
        if 'latitude' in df.columns and 'longitude' in df.columns and 'trip_id' in df.columns:
            if trip_to_plot in df['trip_id'].values:
                trip_points = df[df['trip_id'] == trip_to_plot]
                plt.plot(trip_points['longitude'], trip_points['latitude'], '-', linewidth=2, alpha=0.9, zorder=3, color='blue', label=f'Trip {trip_to_plot}')
                # Collect bounds
                min_lon, max_lon = trip_points['longitude'].min(), trip_points['longitude'].max()
                min_lat, max_lat = trip_points['latitude'].min(), trip_points['latitude'].max()
